...
/**
  ...
  **/
  ...
  double det(double** arr, int n)
  {
 	 ...
 	 double d=0;
  	for(int c=0;c<n;c++){
  		double** smolm =(double**)malloc((n-1)*sizeof(double*));
  		for(int i=0;i<n-1;i++)
  		{ smolm[i]=(double*)malloc ((n-1) * sizeof(double));}
  		
  		for(int i=1;i<n;i++){
  		  int somn=0;
  		  for(int j=0;j<n;j++){
  		     if(j==c)continue;
  		     smolm[i-1][somn]=arr[i][j];
  		     somn++;
  		     }
  		}
  		
  		double smold= det(smolm,n-1);
  		
  		for(int i=0;i<n-1;i++) free(smolm[i]);
  		free(smolm);
